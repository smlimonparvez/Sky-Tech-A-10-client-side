...
const CheckOut = () => {
    ...
    const {title, _id, price, time} = course
    return (
        <div className='shadow-lg mx-auto m-10 w-2/4 p-10 bg-slate-100 rounded'>
            ...
            <p className='mb-2'><span className='font-medium'>Total Course Time:</span> {time}</p>
            <span><hr className='w-3/4 mx-auto my-4'/></span>
            <h3 className='font-medium text-center text-lg'><span>Description</span></h3>
            <p className='text-justify'>Thanks for buying this course. It's contain 40 video. To finish this course you need total almost {time}. Keep patient, keep learning and fulfil your dream.</p>
        </div>
    );
};
